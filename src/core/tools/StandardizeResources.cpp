...
int main() {
    ...
    auto resourceDir = fs::path(AppPaths::dataDir()) / "data" / "resources";

    if (!fs::exists(resourceDir)) {
        std::cerr << "Resource directory does not exist: " << resourceDir
                  << "\n";
        return 1;
    }
    ...
    for (auto const &entry: fs::directory_iterator(resourceDir)) {
        ...
        // 例如 .jpeg → .jpg
        auto subtype =
            wechat::core::subtypeFromExtension(oldPath.extension().string());
        ...
        std::cout << "  RENAME " << oldPath.filename() << " -> " << expectedName
                  << "\n";
        fs::rename(oldPath, newPath);
        ++renamed;
    }

    std::cout << "\nDone: " << renamed << " renamed, " << skipped
              << " unchanged.\n";
    return 0;
}
