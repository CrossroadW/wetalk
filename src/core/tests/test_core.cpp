...
protected:
    SQLite::Database db{"", SQLite::OPEN_READWRITE | SQLite::OPEN_CREATE | SQLite::OPEN_MEMORY};
...
TEST_F(SQLiteTest, InsertAndQueryGroup) {
    ...
    SQLite::Statement insertGroup(db, "INSERT INTO groups_ (id, owner_id) VALUES (?, ?)");
    insertGroup.bind(1, "g1");
    insertGroup.bind(2, "");
    insertGroup.exec();

    SQLite::Statement insertMember(db, "INSERT INTO group_members (group_id, user_id) VALUES (?, ?)");
    ...
    SQLite::Statement countMembers(db, "SELECT COUNT(*) FROM group_members WHERE group_id = ?");
...
TEST_F(SQLiteTest, InsertAndQueryMessage) {
    ...
    insert.bind(6, 1700000000);
    insert.exec();

    SQLite::Statement query(db, "SELECT * FROM messages WHERE chat_id = ? ORDER BY timestamp");
    query.bind(1, "g1");
    ASSERT_TRUE(query.executeStep());
    EXPECT_EQ(query.getColumn("id").getString(), "msg1");
    EXPECT_EQ(query.getColumn("sender_id").getString(), "u1");
    EXPECT_EQ(query.getColumn("content_data").getString(), R"([{"type":1,"data":{"text":"hello"}}])");
    EXPECT_EQ(query.getColumn("timestamp").getInt64(), 1700000000);
...
TEST_F(SQLiteTest, MessageEditAndRevoke) {
    db.exec("INSERT INTO groups_ (id, owner_id) VALUES ('g1', '')");
    db.exec("INSERT INTO messages (id, sender_id, chat_id, content_data, timestamp) "
            "VALUES ('msg1', 'u1', 'g1', '[{\"type\":1,\"data\":{\"text\":\"hello\"}}]', 1700000000)");

    // 编辑
    SQLite::Statement edit(db, "UPDATE messages SET content_data = ?, edited_at = ? WHERE id = ?");
    edit.bind(1, R"([{"type":1,"data":{"text":"hello edited"}}])");
    edit.bind(2, 1700000100);
    edit.bind(3, "msg1");
    edit.exec();

    SQLite::Statement q1(db, "SELECT content_data, edited_at FROM messages WHERE id = ?");
    q1.bind(1, "msg1");
    ASSERT_TRUE(q1.executeStep());
    EXPECT_EQ(q1.getColumn("content_data").getString(), R"([{"type":1,"data":{"text":"hello edited"}}])");
    EXPECT_EQ(q1.getColumn("edited_at").getInt64(), 1700000100);
...
TEST_F(SQLiteTest, MessageReplyTo) {
    db.exec("INSERT INTO groups_ (id, owner_id) VALUES ('g1', '')");
    db.exec("INSERT INTO messages (id, sender_id, chat_id, content_data, timestamp) "
            "VALUES ('msg1', 'u1', 'g1', '[{\"type\":1,\"data\":{\"text\":\"original\"}}]', 1700000000)");
...
TEST_F(SQLiteTest, ReadCountIncrement) {
    db.exec("INSERT INTO groups_ (id, owner_id) VALUES ('g1', '')");
    db.exec("INSERT INTO messages (id, sender_id, chat_id, content_data, timestamp) "
            "VALUES ('msg1', 'u1', 'g1', '[{\"type\":1,\"data\":{\"text\":\"hi\"}}]', 1700000000)");

    db.exec("UPDATE messages SET read_count = read_count + 1 WHERE id = 'msg1'");
    db.exec("UPDATE messages SET read_count = read_count + 1 WHERE id = 'msg1'");
    db.exec("UPDATE messages SET read_count = read_count + 1 WHERE id = 'msg1'");
